...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Prompt user to select a session if any exist
        if self.session_list_widget.count() > 0:
            QMessageBox.information(
                self,
                "No Session Selected",
                "Please double-click a session to select it."
            )
    ...
    def create_keyframes(self):
        ...
        # Get list of available AOIs from aoi_list_widget
        aois = []
        for i in range(self.aoi_list_widget.count()):
            item = self.aoi_list_widget.item(i)
            if item.isSelected():
                aois.append(item.text())
        if not aois:
            QMessageBox.warning(
                self,
                "No AOIs Available",
        ...
        # Prompt user to pick an AOI
        aoi_name, ok = QInputDialog.getItem(
            self,
            "Assign AOI",
            "Select an AOI:",
            aois,
        ...
        with open(session_path, "a", newline="") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow([aoi_name, in_time_str, duration_str, out_time_str])
